...
class Exercicio9Test {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"b_condicionais.Teste2"})
    void testUsoOperadorTernario(String className) throws Exception {
        String path = "src/main/java/" + className.replace('.', '/') + ".java";
        java.nio.file.Path filePath = java.nio.file.Paths.get(path);
        String source = java.nio.file.Files.readString(filePath);
        boolean contemTernario = source.contains("?") && source.contains(":");
        assertTrue(contemTernario, "A implementação da classe '" + className + "' deve utilizar o operador ternário ( ? : )");
    }
